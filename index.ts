...
export class ChaCha20 {
    ...
    /**
     * @param rounds the rounds number of 8, 12, or 20 can be specified.
     */
    private stream(src: Uint8Array, dst: Uint8Array, len: number, rounds: ROUNDS) {
     ...
     */
    encrypt(key: Uint8Array, pt: Uint8Array, iv: Uint8Array, rounds: ROUNDS = 20, cnt: number = 0): Uint8Array {
        let ct = new Uint8Array(pt.length)
        this.init(key, iv, cnt).stream(pt, ct, pt.length, rounds)
        return ct
    }
     ...
     */
    decrypt(key: Uint8Array, ct: Uint8Array, iv: Uint8Array, rounds: ROUNDS = 20, cnt: number = 0): Uint8Array {
        let pt = new Uint8Array(ct.length)
        this.init(key, iv, cnt).stream(ct, pt, ct.length, rounds)
        return pt
    }
